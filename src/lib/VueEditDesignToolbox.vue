<template>
  <v-sheet class="vue-edit-toolbox" elevation="2" outlined dark>
    <v-breadcrumbs divider=">" :items="selectionPathNodes">
      <template #item="{ item }">
        <v-breadcrumbs-item>
          <v-btn elevation="2" @click="requestSelectionChange(item.path)">
            <span style="text-transform: none">{{ item.name }}</span>
          </v-btn>
          <v-btn icon small @click="requestUnwrap(item.path)">
            <v-icon> mdi-border-none-variant </v-icon>
          </v-btn>
          <v-btn icon small @click="requestNodeRemoval(item.path)">
    ...
    <v-container fluid>
      <v-row no-gutters>
        ...
        <v-col class="scroll">
          <v-card>
            <v-card-text>
              <p v-if="!allSelectionSlots.length || selectionType === 'text'">
                ...
                >
                  ...
                  <v-list-item-group color="primary">
                    <v-list-item v-if="slot.children.length > 0">
                      ...
                      />
                      <v-btn icon @click="requestNewNode(slot, 'start')">
                    ...
                    >
                      <v-list-item
                        ...
                        :key="child.path"
                      >
                        <v-icon class="drag-handle"> mdi-dots-vertical </v-icon>
                        <v-list-item-content>
                          [{{ child.index }}] {{ child.name }}
                        </v-list-item-content>
                        <v-btn icon @click="requestSelectionChange(child.path)">
                          <v-icon> mdi-target </v-icon>
                        </v-btn>
                        <v-btn icon @click="requestNodeRemoval(child.path)">
                          <v-icon> mdi-delete </v-icon>
                        </v-btn>
                      </v-list-item>
                    </v-draggable>
                    <v-list-item>
                      ...
                      />
                      <v-btn icon @click="requestNewNode(slot, 'end')">
                        <v-icon> mdi-plus </v-icon>
                      </v-btn>
                    </v-list-item>
                  </v-list-item-group>
        ...
        <v-col>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-btn @click="requestWrapSelection('strong')">
                <v-icon> mdi-format-bold </v-icon>
              </v-btn>
              <v-btn @click="requestWrapSelection('em')">
